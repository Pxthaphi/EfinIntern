...
( function( $ ) {
	...
	*/
	$('.onovo-filter-nav-active').each(function() {
		$(this).css({'width':$(this).closest('.onovo-filter.filter--default').find('.item--active').parent().width()-20+'px'});
	});
	$('.onovo-filter').on( 'click', 'button', function() {
		var filterValue = $(this).attr('data-filter');
		//filter item from container valuable
		...
		//here is padding as you want to adjust like a left+right 16px is 16*2 if u want to adjust specific side 
		$(this).closest('.onovo-filter.filter--default').find('.onovo-filter-nav-active').css({'width':current_width-32+'px'});
		//U needs to be adjust more in position here---
		$(this).closest('.onovo-filter.filter--default').find('.onovo-filter-nav-active').css({'left':current_pos+'px'});
	});
